...
namespace CleanArchitecture.UnitTest
{
    public class CarsControllerUnitTest
    {
        ...
        public async void Create_ReturnsOkResult_WhenRequestIsValid()
        {
            ...
            MessageResponse response = new("Araç baþarýyla kayýt edildi!");
            CancellationToken cancellationToken = new();
            mediatorMock.Setup(m => m.Send(createCarCommand, cancellationToken))
                .ReturnsAsync(response);
            CarsController carsControllers = new(mediatorMock.Object);
            //Act
            var result = await carsControllers.Create(createCarCommand, cancellationToken);
            //Assert
